...
// src/tranform.ts
var prefixUrl = "https://bscscan.com";
function tranform_default(data2) {
  const { totaleth, totalusd, recordsfound, layout } = data2;
  ...
  const list = $2("tr").map((i, $tr) => {
    ...
    return {
      ...
      totalusd,
      total,
      pic: prefixUrl + $2(TokenName).find(".u-sm-avatar").attr("src"),
      TokenName: _TokenName,
      TokenHash: _TokenHash,
      Symbol: $2(Symbol2).text(),
      Quantity: $2(Quantity).text(),
      TokenPrice: $2(TokenPrice).text(),
      Change24h: $2(Change24h).text(),
      ValueIInBNB: $2(ValueIInBNB).text(),
      ValueInUSD: $2(ValueInUSD).text()
    };
  }).toArray();
